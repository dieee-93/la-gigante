...
public class CategoriasProductoDAOImpl implements CategoriasProductoDAO {
	...
	public int insert(Categoria cat) {
		try {
			String sql = "INSERT INTO CATEGORIAS_PRODUCTO (NOMBRE, CATEGORIA_PADRE) VALUES (?, ?)";
			Connection conn = ConnectionProvider.getConnection();
	...
	public int update(Categoria cat) {
		try {
			String sql = "UPDATE CATEGORIAS_PRODUCTO SET NOMBRE = ?, CATEGORIA_PADRE = ?, WHERE ID = ?";
			Connection conn = ConnectionProvider.getConnection();
	...
	public int countAll() {
		try {
			...
			String sql = "SELECT COUNT(1) AS TOTAL FROM CATEGORIAS_PRODUCTO";
			Connection conn = ConnectionProvider.getConnection();
			PreparedStatement statement = conn.prepareStatement(sql);
			ResultSet resultados = statement.executeQuery();
